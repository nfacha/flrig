...
unsigned long long RIG_FT991::get_vfoA ()
{
	cmd = rsp = "FA";
	cmd += ';';
...
void RIG_FT991::set_vfoA (unsigned long long freq)
{
	freqA = freq;
	cmd = "FA000000000;";
...
unsigned long long RIG_FT991::get_vfoB ()
{
	cmd = rsp = "FB";
	cmd += ';';
...
void RIG_FT991::set_vfoB (unsigned long long freq)
{
	freqB = freq;
	cmd = "FB000000000;";
	for (int i = 0; i < ndigits; i++) {
...
double RIG_FT991::get_power_control()
{
	cmd = rsp = "PC";
	cmd += ';';
...
void RIG_FT991::get_vfoadj_min_max_step(double &min, double &max, double &step)
{
	min = -25;
	max = 25;
	step = 1;
...
void RIG_FT991::set_squelch(int val)
{
	cmd = "SQ0000;";
	for (int i = 5; i > 2; i--) {
		cmd[i] = val % 10 + '0';
...
int  RIG_FT991::get_squelch()
{
	int sqval = 0;
	cmd = rsp = "SQ0";
	cmd += ';';
	get_trace(1, "get_squelch()");
	wait_char(';',7, FL991_WAIT_TIME, "get squelch", ASC);
	gett("");
