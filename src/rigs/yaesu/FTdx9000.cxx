...
// 19,   19,    9,   0,   0,  10,       9,    15,     10,       0,        0,    15,     0	// FTdx9000_def_bw
...
//----------------------------------------------------------------------
static std::vector<std::string>FTdx9000_att_labels;
static const char *vFTdx9000_att_labels[] = { "ATT", "6 dB", "12 dB", "18 dB" };

static std::vector<std::string>FTdx9000_pre_labels;
static const char *vFTdx9000_pre_labels[] = { "IPO", "Amp 1", "Amp 2" };

static std::vector<std::string>FTdx9000_nb_labels;
static const char *vFTdx9000_nb_labels[] = { "NB", "NB 1", "NB 2" };
//----------------------------------------------------------------------
...
void RIG_FTdx9000::initialize()
{
	...
	VECTOR (FTdx9000_att_labels, vFTdx9000_att_labels);
	att_labels_ = FTdx9000_att_labels;

	VECTOR (FTdx9000_pre_labels, vFTdx9000_pre_labels);
	pre_labels_ = FTdx9000_pre_labels;

	VECTOR (FTdx9000_nb_labels, vFTdx9000_nb_labels);
	nb_labels_ = FTdx9000_nb_labels;
...
unsigned long long RIG_FTdx9000::get_vfoA ()
{
	cmd = rsp = "FA";
	cmd += ';';
...
void RIG_FTdx9000::set_vfoA (unsigned long long freq)
{
	freqA = freq;
	cmd = "FA00000000;";
	for (int i = 9; i > 1; i--) {
...
unsigned long long RIG_FTdx9000::get_vfoB ()
{
	cmd = rsp = "FB";
	cmd += ';';
...
void RIG_FTdx9000::set_vfoB (unsigned long long freq)
{
	freqB = freq;
	cmd = "FB00000000;";
	for (int i = 9; i > 1; i--) {
...
bool RIG_FTdx9000::twovfos()
{
	return true;
}

void RIG_FTdx9000::selectA()
{
	cmd = "FR0;FT2;";
	sendCommand(cmd);
...
bool RIG_FTdx9000::can_split()
{
	return true;
}

void RIG_FTdx9000::set_split(bool val)
{
	split = val;
	if (val) {
		cmd = "FR0;";
...
int RIG_FTdx9000::get_split()
{
	return split;
}


int RIG_FTdx9000::get_smeter()
{
	cmd = rsp = "SM0";
	cmd += ';';
...
void RIG_FTdx9000::set_power_control(double val)
{
	int ival = (int)val;
	cmd = "PC000;";
	for (int i = 4; i > 1; i--) {
...
int RIG_FTdx9000::get_volume_control()
{
	cmd = rsp = "AG0";
	cmd += ';';
...
void RIG_FTdx9000::set_volume_control(int val) 
{
...
void RIG_FTdx9000::set_PTT_control(int val)
{
	cmd = val ? "TX1;" : "TX0;";
	sendCommand(cmd);
...
void RIG_FTdx9000::tune_rig(int val)
{
	switch (val) {
		case 0:
			cmd = "AC000;";
...
int  RIG_FTdx9000::next_attenuator()
{
	switch (atten_state) {
		case 0: return 1;
		case 1: return 2;
...
void RIG_FTdx9000::set_attenuator(int val)
{
	atten_state = val;
...
int RIG_FTdx9000::get_attenuator()
{
	cmd = rsp = "RA0";
	cmd += ';';
...
void RIG_FTdx9000::set_preamp(int val)
{
	preamp_state = val;
	cmd = "PA00;";
...
int RIG_FTdx9000::adjust_bandwidth(int val)
{
	int bw = 0;
	if (val == mCW || val == mCW_R) {
		bandwidths_ = FTdx9000_widths_CW;
		bw_vals_ = FTdx9000_wvals_CW;
	} else if (val == mFM || val == mAM || val == mFM_N || val == mPKT_FM || val == mAM_N) {
		if (val == mFM) bandwidths_ = FTdx9000_widths_FMwide;
		else if (val ==  mAM) bandwidths_ = FTdx9000_widths_AMwide;
		else if (val == mAM_N) bandwidths_ = FTdx9000_widths_AMnar;
		else if (val == mFM_N) bandwidths_ = FTdx9000_widths_FMnar;
		else if (val == mPKT_FM) bandwidths_ = FTdx9000_widths_FMpkt;
		bw_vals_ = FTdx9000_wvals_AMFM;
	} else if (val == mRTTY_L || val == mRTTY_U) { // RTTY
		bandwidths_ = FTdx9000_widths_RTTY;
		bw_vals_ = FTdx9000_wvals_RTTY;
	} else if (val == mPKT_L || val == mPKT_U) { // PSK
		bandwidths_ = FTdx9000_widths_PSK;
		bw_vals_ = FTdx9000_wvals_PSK;
	} else {
		bandwidths_ = FTdx9000_widths_SSB;
		bw_vals_ = FTdx9000_wvals_SSB;
	}
	bw = FTdx9000_def_bw[val];
	return bw;
}
...
std::vector<std::string>& RIG_FTdx9000::bwtable(int n)
{
	switch (n) {
		...
		case mFM: return FTdx9000_widths_FMwide;
		case mAM: return FTdx9000_widths_AMwide;
		case mAM_N : return FTdx9000_widths_AMnar;
		case mRTTY_L: case mRTTY_U: return FTdx9000_widths_RTTY;
		case mPKT_L: case mPKT_U: return FTdx9000_widths_PSK;
		case mFM_N: return FTdx9000_widths_FMnar;
		case mPKT_FM: return FTdx9000_widths_FMpkt;
		default: ;
	}
	return FTdx9000_widths_SSB;
}

void RIG_FTdx9000::set_modeA(int val)
{
	modeA = val;
	cmd = "MD0";
	cmd += FTdx9000_mode_chr[val];
	cmd += ';';
	sendCommand(cmd);
...
void RIG_FTdx9000::set_modeB(int val)
{
	modeB = val;
	cmd = "MD0";
	cmd += FTdx9000_mode_chr[val];
...
void RIG_FTdx9000::set_bwA(int val)
{
...
void RIG_FTdx9000::set_bwB(int val)
{
	int bw_indx = bw_vals_[val];
	bwB = val;
...
std::string RIG_FTdx9000::get_BANDWIDTHS()
{
	std::stringstream s;
...
void RIG_FTdx9000::set_BANDWIDTHS(std::string s)
{
...
int RIG_FTdx9000::get_modetype(int n)
{
	return FTdx9000_mode_type[n];
}

void RIG_FTdx9000::set_if_shift(int val)
{
	cmd = "IS0+0000;";
...
void RIG_FTdx9000::get_if_min_max_step(int &min, int &max, int &step)
{
...
void RIG_FTdx9000::set_notch(bool on, int val)
{
	if (on && !notch_on) {
		notch_on = true;
		cmd = "BP00001;";
...
bool  RIG_FTdx9000::get_notch(int &val)
{
	bool ison = false;
	cmd = rsp = "BP00";
	cmd += ';';
...
void RIG_FTdx9000::get_notch_min_max_step(int &min, int &max, int &step)
{
	min = 10;
	max = 3000;
	step = 10;
...
void RIG_FTdx9000::set_auto_notch(int v)
{
	cmd = "BC00;";
	if (v) cmd[3] = '1';
	sendCommand(cmd);
...
int  RIG_FTdx9000::get_auto_notch()
{
	cmd = "BC0;";
...
void RIG_FTdx9000::set_noise(bool b)
{
	cmd = "NB00;";
	if (nb_state == 0) {
		nb_state = 1;
...
int RIG_FTdx9000::get_noise()
{
	...
	if (nb_state == 1) {
		noise_blanker_label("NB 1", true);
	} else if (nb_state == 2) {
		noise_blanker_label("NB 2", true);
	} else {
...
// val 0 .. 100
void RIG_FTdx9000::set_mic_gain(int val)
{
	cmd = "MG000;";
...
void RIG_FTdx9000::get_mic_min_max_step(int &min, int &max, int &step)
{
	min = 0;
	max = 100;
	step = 1;
...
void RIG_FTdx9000::set_rf_gain(int val)
{
	cmd = "RG0000;";
...
void RIG_FTdx9000::get_rf_min_max_step(int &min, int &max, int &step)
{
	min = 0;
	max = 100;
	step = 1;
...
void RIG_FTdx9000::set_vox_onoff()
{
	cmd = "VX0;";
...
void RIG_FTdx9000::set_vox_gain()
{
	cmd = "VG";
...
void RIG_FTdx9000::set_vox_anti()
{
	cmd = "EX175";
...
void RIG_FTdx9000::set_vox_hang()
{
	cmd = "VD";
...
void RIG_FTdx9000::set_vox_on_dataport()
{
	cmd = "EX1140;";
...
void RIG_FTdx9000::enable_keyer()
{
	cmd = "KR0;";
...
void RIG_FTdx9000::set_cw_weight()
{
	int n = round(progStatus.cw_weight * 10);
	cmd.assign("EX056").append(to_decimal(n, 2)).append(";");
	sendCommand(cmd);
	showresp(WARN, ASC, "SET cw weight", cmd, replystr);
}

void RIG_FTdx9000::set_cw_qsk()
{
	int n = progStatus.cw_qsk / 5 - 3;
	cmd.assign("EX059").append(to_decimal(n, 1)).append(";");
	sendCommand(cmd);
