...
static std::vector<std::string>TS480HXmodes_;
static const char *vTS480HXmodes_[] =
...
static std::vector<std::string>TS480HX_empty;
static const char *vTS480HX_empty[] =
{ "N/A" };
static int TS480HX_bw_vals[] = {1, WVALS_LIMIT};

// SL command is lo cut when menu 045 OFF
static std::vector<std::string>TS480HX_SL;
static const char *vTS480HX_SL[] =
{
  "0",   "50", "100", "200", "300", 
"400",  "500", "600", "700", "800", 
"900", "1000" };
...
static std::vector<std::string>TS480HX_SH;
static const char *vTS480HX_SH[] =
{
...
static std::vector<std::string>TS480HX_dataW;
static const char *vTS480HX_dataW[] =
...
static std::vector<std::string>TS480HX_dataC;
static const char *vTS480HX_dataC[] =
{ "1000", "1500", "2210" };
static const char *TS480HX_dataC_tooltip = "center";
static const char *TS480HX_dataC_label = "C";

static std::vector<std::string>TS480HX_AM_SL;
static const char *vTS480HX_AM_SL[] =
{ "10", "100", "200", "500"};

static std::vector<std::string>TS480HX_AM_SH;
static const char *vTS480HX_AM_SH[] =
{ "2500", "3000", "4000", "5000"};
//static int TS480HX_AM_bw_vals[] = {1,2,3,4,WVALS_LIMIT};

static std::vector<std::string>TS480HX_CWwidths;
static const char *vTS480HX_CWwidths[] =
{
"50", "80", "100", "150", "200", 
...
static std::vector<std::string>TS480HX_CWbw;
static const char *vTS480HX_CWbw[] =
{
"FW0050;", "FW0080;", "FW0100;", "FW0150;", "FW0200;", 
"FW0300;", "FW0400;", "FW0500;", "FW0600;", "FW1000;", 
"FW2000;" };

static std::vector<std::string>TS480HX_FSKwidths;
static const char *vTS480HX_FSKwidths[] =
{
"250", "500", "1000", "1500"};
static int TS480HX_FSK_bw_vals[] = { 1,2,3,4,WVALS_LIMIT};

static std::vector<std::string>TS480HX_FSKbw;
...
{
	VECTOR (TS480HXmodes_, vTS480HXmodes_);
	VECTOR (TS480HX_empty, vTS480HX_empty);
	VECTOR (TS480HX_SL, vTS480HX_SL);
	VECTOR (TS480HX_SH, vTS480HX_SH);
	VECTOR (TS480HX_dataW, vTS480HX_dataW);
	VECTOR (TS480HX_dataC, vTS480HX_dataC);
	VECTOR (TS480HX_AM_SL, vTS480HX_AM_SL);
	VECTOR (TS480HX_AM_SH, vTS480HX_AM_SH);
	VECTOR (TS480HX_CWwidths, vTS480HX_CWwidths);
	VECTOR (TS480HX_CWbw, vTS480HX_CWbw);
	VECTOR (TS480HX_FSKwidths, vTS480HX_FSKwidths);
	VECTOR (TS480HX_FSKbw, vTS480HX_FSKbw);
...
const char * RIG_TS480HX::get_bwname_(int n, int md) 
{
	...
	if (n > 256) {
		...
		snprintf(bwname, sizeof(bwname), "%s/%s",
			(md == 0 || md == 1 || md == 3) ? dsp_SL[lo].c_str() : TS480HX_AM_SL[lo].c_str(),
			(md == 0 || md == 1 || md == 3) ? dsp_SH[hi].c_str() : TS480HX_AM_SH[hi].c_str() );
	} else {
		snprintf(bwname, sizeof(bwname), "%s",
			(md == 2 || md == 6) ? TS480HX_CWwidths[n].c_str() : TS480HX_FSKwidths[n].c_str());
	}
	return bwname;
}
...
std::vector<std::string>& RIG_TS480HX::bwtable(int m)
{
	if (m == 0 || m == 1 || m == 3)
...
std::vector<std::string>& RIG_TS480HX::lotable(int m)
{
	if (m == 0 || m == 1 || m == 3)
		return TS480HX_SL;
	else if (m == 2 || m == 6)
...
std::vector<std::string>& RIG_TS480HX::hitable(int m)
{
	if (m == 0 || m == 1 || m == 3)
		return TS480HX_SH;
	else if (m == 2 || m == 6)
